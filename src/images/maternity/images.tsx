import { galleryItem } from '../../pages/GalleryFunctions'
import image01 from './image01.jpg';
import image02 from './image02.jpg';
import image03 from './image03.jpg';
import image04 from './image04.jpg';
import image05 from './image05.jpg';
import image06 from './image06.jpg';
import image07 from './image07.jpg';
import image08 from './image08.jpg';
import image09 from './image09.jpg';
import image10 from './image10.jpg';


export const maternityImages : galleryItem[] = [
    { img: image01, alt: 'maternity1'},
    { img: image02, alt: 'maternity2'},
    { img: image03, alt: 'maternity3'},
    { img: image04, alt: 'maternity4'},
    { img: image06, alt: 'maternity6', wide: true},
    { img: image08, alt: 'maternity8'},
    { img: image07, alt: 'maternity7'},
    { img: image10, alt: 'maternity10'},
    { img: image05, alt: 'maternity5', wide: true},
    { img: image09, alt: 'maternity9'},
  ];